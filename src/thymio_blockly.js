...
$().ready(function () {
  ...
  client.onNodesChanged = async (nodes) => {
    ...
    for (let node of nodes) {
      console.log(
        `${node.id} : ${node.name} ${node.typeAsString}  ${node.statusAsString}`
      );
      // Select the first non busy node
      if (
        (!selectedNode || !selectedNode.isReady) &&
        node.status == NodeStatus.available &&
        (!preferredNodeId || preferredNodeId == node.id.toString())
      ) {
        try {
          selectedNode = node;
          await node.lock();
...
function start() {
  ...
  let loadFile = function (fileLoadedEvent, name) {
    ...
    Blockly.Xml.domToWorkspace(
      workspace,
      xmlDoc.getElementsByTagName("xml")[0]
    );
